...
We are resubmitting TipTrack Delivery Log 1.0 with build 16 and need help verifying why the non-consumable in-app purchase is not being returned by StoreKit in sandbox/App Review.
...
- The Bundle ID has the In-App Purchase capability enabled, and the submitted Xcode target explicitly enables the In-App Purchase capability.
...
- Build 16 is valid and attached to version 1.0.
...
Build 16 includes bounded StoreKit product-loading retries and a timeout fallback. On a physical iPhone using the sandbox environment, the app still receives no product for `com.steveafrost.tiptrack.pro.unlock`, so the Pro screen cannot show the App Store purchase button.
